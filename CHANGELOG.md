...
- Aprendizados do lote de 14 a 20/09 (15 clientes reais): nova seção "Checagens de fato antes de entregar um rascunho ou aprovar um texto" (plano vs estado atual da conta, status e link de cada anúncio citado, amostra pequena, semana anterior incompleta, evento de otimização, ação de outro membro da equipe, não inventar P2, não afirmar mecanismos da Meta, reescrever quando o gestor traz o motivo real, alerta crítico primeiro). Mais: como medir quanto tempo a conta ficou parada por pagamento (gasto por dia + recargas) e calcular quantos dias o saldo dura, horários do log em UTC (converter pra Brasília), pegadinha do zsh com variáveis sem aspas, e não usar "mais seguidores" como critério de melhor anúncio (a Meta não dá seguidor por anúncio).
