...
## Checagem de atualização da skill (no começo de cada pedido novo, no máximo 1 vez por dia)

Toda vez que o gestor começar um **pedido novo** com esta skill (um novo cliente, um novo lote, uma nova semana), antes de trabalhar, veja rapidamente se existe versão nova no GitHub e avise. Isso vale **mesmo numa conversa antiga que ele nunca fecha**: o que limita a frequência não é a conversa, é um marcador de data (`.ultima-checagem`, ignorado pelo git) que impede de checar mais de uma vez por dia. Não rode isso a cada mensagem dentro de um mesmo pedido.

1. Rode este comando único, da pasta da skill: `cd ~/.claude/skills/avaliador-checkin-relatorio-c2g && if [ -z "$(find .ultima-checagem -mmin -1440 2>/dev/null)" ]; then git fetch -q origin && touch .ultima-checagem && git rev-list --count HEAD..origin/main; else echo skip; fi`
2. **Pule em silêncio, sem avisar nem atrapalhar**, se o resultado for `skip` (já checou nas últimas 24h) ou `0` (já está atualizada), ou se der erro/demorar (pasta que não é repositório git por ter sido instalada por zip, ou sem internet).
3. Se o resultado for maior que 0, mostre em 2 a 3 linhas o que mudou (`git log --oneline HEAD..origin/main`, traduzindo pro gestor em linguagem simples) e **pergunte se ele quer atualizar agora**. Siga com o pedido dele normalmente, sem esperar a resposta pra trabalhar. Se ele disser que não, não insista nessa mesma conversa.
...
- **Pegadinhas de shell e horário:** no zsh (padrão do macOS) uma variável sem aspas **não** é separada em palavras, então `for R in "a b"; do set -- $R` não funciona: use variáveis separadas ou `cut`. E os horários do log de atividades vêm em **UTC**: converta pra Brasília (-3h) antes de escrever uma hora ou um dia no texto (um evento perto da meia-noite muda de dia).
...
4. `ad_account_update_status` (com `actor_name: "Meta"`) = **mudança no status de pagamento/saúde da conta** (ex.: de "Ativa" pra "Pagamento necessário") — isso é crítico e sempre deve ir pro check-in como ponto de atenção urgente, não pode passar batido, mesmo sendo um evento do sistema. Se aparecer, confirme com `/act_<ID>?fields=account_status,disable_reason,funding_source_details` pra ver o status atual e a forma de pagamento cadastrada, e avise o Well claramente (a menos que ele peça pra ignorar por enquanto). **Pra medir quanto tempo a conta ficou parada:** puxe o gasto por dia (`insights` com `time_increment=1`) e os eventos `funding_event_successful` (recargas, ex.: PIX) do log. Dias com gasto perto de zero junto com o status "Pagamento necessário" mostram exatamente os períodos parados, e isso costuma ser a principal causa de queda de volume da semana. Em conta pré-paga ou paga por recarga, calcule quantos dias o saldo dura (saldo dividido pelo gasto diário) e considere um pedido leve ao cliente pra deixar um saldo maior de uma vez, sem contar quantas vezes já aconteceu.
...
3. Pra cada anúncio escolhido, pegue o permalink do **Instagram** (não do Facebook) direto do criativo: `/<ad_id>?fields=creative{instagram_permalink_url}`. Isso já devolve a URL pronta tipo `https://www.instagram.com/p/<código>/` — não precisa montar nada manualmente.
   **Cuidado com o critério:** a Meta não devolve seguidores por anúncio, então nunca use "mais seguidores" como etiqueta; use visitas ao perfil, cliques ou conversas. O anúncio apontado como o melhor precisa bater com o 1º lugar do PDF naquele critério, ou o texto explica por que não (ex.: foi pausado no meio da semana). Se o mesmo criativo existe em várias cópias, some os resultados e use um link só.
...
## Checagens de fato antes de entregar um rascunho ou aprovar um texto

Rode estas conferências na conta antes de aprovar um texto do gestor ou de entregar um rascunho. Todas nasceram de erros reais que passariam batido:

- **O plano bate com o estado atual da conta?** Confira o `effective_status` das campanhas e anúncios citados no P5 e veja se há eventos **depois do fim do período** (hoje). Já aconteceu de o texto dizer "seguir monitorando a campanha que está performando bem" enquanto ela tinha sido pausada naquele mesmo dia (por causa de uma configuração de CRM). Se o gestor pausou/mudou algo depois do período, o texto precisa dizer, com o motivo.
- **Cada anúncio citado:** o status (reprovado, pausado, uma cópia ainda ativa?), o permalink resolvido, e se o link é da cópia que realmente teve o resultado. Não cite pelo nome um anúncio cujo link você não consegue confirmar, e não afirme "pausei o anúncio X" se só uma das cópias foi pausada.
- **Amostra pequena não é destaque.** "Menor custo da semana" com 1 ou 2 conversas não prova nada: escreva "pouco dado" ou não destaque.
- **Semana anterior incompleta.** Antes de comemorar um +% grande, veja quantos dias a campanha/conta rodou nas duas semanas (campanha nova, conta parada por pagamento). Diga no texto quando parte do crescimento vem de ter rodado a semana inteira.
- **Campanha otimizada por um evento.** Se a campanha otimiza por um evento personalizado (ex.: "qualificado", "valores enviados"), confira `promoted_object` no conjunto de anúncios e diga qual é, deixando claro a diferença entre o que a campanha persegue e o que o relatório conta (conversas iniciadas).
- **Quem fez a ação.** Uma ação de outro membro da equipe no log não é do gestor: escreva "subimos"/"a equipe subiu", não "subi", ou confirme com ele.
- **Sem ação no log = sem P2 inventado.** Se o log não mostra nada do gestor na semana, não crie uma ação. Se o que houve foi deixar um teste rodando de propósito, descreva isso só depois de ele confirmar.
- **Não afirme mecanismos da Meta sem certeza.** Ex.: a fase de aprendizado é do conjunto de anúncios (entrar vários anúncios de uma vez reinicia o do conjunto), não de cada criativo.
- **Quando o gestor mostra print, áudio ou transcrição** (feedback do cliente, reunião, conversa de WhatsApp), isso costuma revelar o motivo real de uma ação. Se ele disser "na verdade foi por isso", **reescreva a narrativa e o plano** em vez de remendar o texto (ex.: se um criativo foi pausado por atrair lead desqualificado, o plano não pode ser reativá-lo). Use o dado do cliente refazendo a conta com os números reais, e mantenha o tom construtivo: nunca reproduza tensão ou crítica interna da reunião no texto do cliente.
- **Alerta crítico primeiro.** Conta parada por pagamento, ou campanhas pausadas até um ajuste externo (CRM, tracking, pagamento), entra logo no início do texto e vem com um pedido leve ao cliente se ele precisar agir.
